...
class RequestWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Text("Оставить заявку", style: AppTextTheme.mediumTextBlack),
        ...
        Table(border: TableBorder(verticalInside: BorderSide()), columnWidths: {
          ...
          TableRow(
              ...
              children: [
                _button(
                    onPressed: () => context.read<PaymentCubit>().minus(),
                    leftRight: false),
                Padding(
                  padding: const EdgeInsets.only(top: 10),
                  child: Text(sum.sum.toString(),
                      textAlign: TextAlign.center,
                      style: AppTextTheme.smallTextMediumBlack),
                ),
                _button(
                    onPressed: () => context.read<PaymentCubit>().plus(),
                    leftRight: true),
              ]),
        ]),
        const SizedBox(height: 20),
        Text("Итого", style: AppTextTheme.smallTextMediumBlack),
        const SizedBox(height: 10),
        Text(sum.total.toString(),
            style: AppTextTheme.splashFirstText.copyWith(color: Colors.black)),
      ],
    );
  }
