  ...
  Widget build(BuildContext context) {
    return BlocConsumer<ResumeUserBloc, ResumeUserState>(listener: (_, state) {
      state.maybeMap(
          orElse: () => state,
          loaded: (_loaded) {
            if (_loaded.status == RESUME_USER_BLOC_GET_RESUME_FAILURE ||
                _loaded.status ==
                    RESUME_USER_BLOC_ACTIVE_OR_DEACTIVATE_FAILURE ||
                _loaded.status == RESUME_USER_BLOC_ADD_FILE_FAILURE ||
                _loaded.status == RESUME_USER_BLOC_CHANGE_EXTRA_BLOCK_FAILURE ||
                _loaded.status == RESUME_USER_BLOC_CHANGE_RESUME_FAILURE ||
                _loaded.status == RESUME_USER_BLOC_ADD_EXTRA_BLOCKS_FAILURE ||
          ...
          noResume: (_state) {
            if (_state.status == RESUME_USER_BLOC_POST_RESUME_PROGRESS) {
              SmallWidgets.showDialogFunction(
                  context: context, title: "Опубликовывается резюме...");
            }
            if (_state.status == RESUME_USER_BLOC_POST_RESUME_IS_NOT_VALIDATE) {
              SmallWidgets.scaffoldMessage(
                  context: context, message: "Заполните все строки");
            }

            if (_state.status == RESUME_USER_BLOC_POST_RESUME_SUCCEED) {
              _function(context);
            }
            if (_state.status == RESUME_USER_BLOC_POST_RESUME_FAILURE) {
              SmallWidgets.scaffoldMessage(
                  context: context, message: "Нет доступ к интернету");
            }
            if(_state.status==RESUME_USER_BLOC_ADD_FILE_SUCCEED){
              SmallWidgets.scaffoldMessage(
                  context: context, message: "Файл закреплен");
            }
          });
    }, builder: (context, state) {
      return state.map(
                  ...
                  children: [
                    if (attributes.resume.name == EMPTY_TITLE_RESUME)
                      Container(
                        ...
                        child: Text("Добавьте первую резюме!",
  ...
  Future<void> _function(BuildContext context) {
    return Future.sync(() => context.read<ResumesUserBloc>().add(
        ResumesUserEvent.addOrDeleteLocalResume(
            nameResume: attributes.resume.name,
            delete: true))).whenComplete(() {
      context.read<ResumeUserBloc>().add(const ResumeUserEvent.getResume());
      context.read<ResumesUserBloc>().add(const ResumesUserEvent.getResumes());
